...
public class CharacterAnalyser : MonoBehaviour
{
    ...
    private readonly string _serverUrl = "http://localhost:5000/extract_characters";
    ...
    // ── Lifecycle ─────────────────────────────────────────────────────────────
    private void Start() => StartCoroutine(LaunchAndAnalyse());
    ...
    private bool LaunchServer()
    {
        ...
        if (!File.Exists(PythonExecutable))
        {
            Debug.LogError($"[CharacterAnalyser] venv Python not found: {PythonExecutable}\n" +
                           "Run setup_nlp.bat once to create it.");
            return false;
        }
